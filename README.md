...
你是一名地球伦理部的调查员，被派往外行星科考站「赫利俄斯」调查一起工程师重伤事故。三台机器人在场，三组自洽的证词，一个看似不可能的逻辑死局。你必须提交责任认定报告。
...
- **内容驱动的阶段推进** — 无倒计时压力：聊完所有角色解锁交叉验证，指控被回应或触及第零法则后进入最终裁决，节奏由你掌控
- **多结局分支** — 自由撰写调查报告，语义引擎自动判定结局走向（含隐藏的第零法则觉醒结局）
- **指控与不指控** — 收集 10 条线索可正式指控任意角色；收集 20 条线索可认定「不指控任何人，问题在规则本身」
- **3 存档位** — 在关键分叉点保存进度，探索不同结局；退出时提醒保存
- **响应式布局** — 同时支持桌面浏览器和 Android 手机
...
点击左侧的「可对话对象」，与三台机器人交流，收集线索，提交你的责任认定报告。
